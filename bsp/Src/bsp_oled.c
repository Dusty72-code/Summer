...
void BSP_OLED_ShowChar(uint8_t x, uint8_t y, char ch) {
    ...
    const uint8_t *font = F6x8[c - 0x20];
    for (uint8_t i = 0; i < 6; i++) {
        for (uint8_t bit = 0; bit < 8; bit++) {
            if (font[i] & (1 << bit)) {
                BSP_OLED_DrawPoint(x + i, y * 8 + bit, 1);
            }
        }
    }
}
...
void BSP_OLED_ShowString(uint8_t x, uint8_t y, const char *str) {
    uint8_t i = 0;
    while (str[i] && (x + i * 6) < OLED_WIDTH) {
        BSP_OLED_ShowChar(x + i * 6, y, str[i]);
        i++;
    }
}
...
void BSP_OLED_ShowNum(uint8_t x, uint8_t y, int32_t num, uint8_t width) {
    ...
    for (int j = t - 1; j >= 0; j--) buf[len++] = tmp[j];
    while ((uint8_t)len < width) buf[len++] = ' ';
    buf[len] = '\0';
...
void BSP_OLED_DrawPoint(uint8_t x, uint8_t y, uint8_t color) {
    ...
    uint8_t page = y / 8;
    uint8_t bit = y % 8;
    if (color) OLED_GRAM[page][x] |= (1 << bit);
